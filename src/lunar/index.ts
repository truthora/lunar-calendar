 ...
 */
  ...
  */
 function phase_hunt_inner(sdate) {
   if (!sdate) {
     sdate = new Date();
   }
   ...
   adate = (nt1 + SYNODIC_MONTH) as any;
   let k2 = k1 + 1;
   let nt2 = meanphase(adate, k2);
   while (nt1 > sdate || sdate >= nt2) {
     adate += SYNODIC_MONTH as any;
     k1++;
     k2++;
     nt1 = nt2;
   ...
   return {
     ...
     nextfull_date: truephase(k2, FULL)
   };
 }
 ...
 function phase_hunt(sdate) {
   const phase = phase_hunt_inner(sdate);
 
   let prevDate = phase.new_date;
   prevDate.setDate(prevDate.getDate() - 1);
   let prevPhase = phase_hunt_inner(prevDate);
 
   let nextPhase = phase_hunt_inner(phase.nextfull_date);
 ...
 const full_moons_before = (date, numberBefore = 0) => {
   let phase = phase_hunt(date);
   let fullMoon = phase.full_date;
   date.setHours(0, 0, 0, 0);
   fullMoon.setHours(0, 0, 0, 0);
 
   if (fullMoon.getTime() >= date.getTime()) {
     fullMoon = phase.prevPhase.full_date;
   }
 
   phase = phase_hunt(fullMoon);
   const fullMoons = [fullMoon];
   let prevPhase = phase.prevPhase.full_date;
   while (numberBefore > 1) {
     phase = phase_hunt(prevPhase);
     fullMoons.push(phase.full_date);
     prevPhase = phase.prevPhase.full_date;
     numberBefore = numberBefore - 1;
   }
   return fullMoons;
 };
 
 const full_moons_after = (date, numberAfter = 0) => {
   let phase = phase_hunt(date);
 
   date.setHours(0, 0, 0, 0);
   phase.full_date.setHours(0, 0, 0, 0);
   const fullMoon =
     phase.full_date.getTime() <= date.getTime()
       ? phase.nextPhase.full_date
       : phase.full_date;
 
   phase = phase_hunt(fullMoon);
   const fullMoons = [fullMoon];
   let nextPhase = phase.nextPhase.full_date;
   while (numberAfter > 1) {
     phase = phase_hunt(nextPhase);
     fullMoons.push(phase.full_date);
     nextPhase = phase.nextPhase.full_date;
 ...
 const getLunarFirstDay = date => {
   ...
   while (holdDate.getFullYear() !== date.getFullYear()) {
     ...
     const nextLeapYearTester = new Date(theLeapTester);
     nextLeapYearTester.setFullYear(nextLeapYearTester.getFullYear() + 1);
     const isLeapYear =
       phase_range(theLeapTester, nextLeapYearTester, FULL).length === 13;
 
     const newFirstYear = getFullMoon(holdDate, isLeapYear ? 13 : 12).pop();
     holdDate = new Date(newFirstYear ?? '');
   }
   return holdDate;
 };
...
export { phase, phase_hunt, phase_range, full_moons_after, full_moons_before, getLunarFirstDay };
